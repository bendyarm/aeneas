...
| `aes_fixslice_keyround.lisp` | handwritten | **Phase 4 — one key_round == the window transform `krw8`, for all inputs**: composes the op layer into a single characterisation — `key-round-window` (`rkeys[off+8..off+16) := krw8(rkeys[off..off+8), c)`), `key-round-car`/`-len`/`-frame-below` (offset `off+8`, length preserved, everything below `off+8` — all earlier round keys — untouched), and `wstatep-of-krw8` (the new window is again a `wstate`, the loop invariant the chain threads). `krw8` is **offset-independent** — exactly the transform the `keycore` step cruxes prove equals the AES key-expansion recurrence at `off=0`. Remaining: `krw8-crux-c` (crux + `key-round-window` ⇒ `inv_bitslice(krw8(bitslice(b,b),c)) = kr-spec-bytes`), the 10-round chain, and the fold-layer combination (roadmap in-book) |
| `aes_fixslice_keycrux.lisp` | handwritten | **Phase 4 — bridges toward `krw8` ↔ spec recurrence**: three reusable, proven bridges — `take-8-nthcdr = rd8` (the cruxes read the window as `take`/`nthcdr`), `bitslice`-of-`inp` is a `wstate`, `rd8` of the seed reads back the `bitslice`. The payoff `krw8-crux-c` (`car(inv_bitslice(krw8(bitslice(b,b),c))) = kr-spec-bytes(b, xpow c)`) is **not yet proven** for any `c`: the window substitution lines up, but doing it in one shot with lifting the crux's sixteen `unsigned-byte-p` byte-hyps to `(inp b)` blows up. The fix is to separate the two steps (lift the crux to `(inp b)` with the codebase's `-general` idiom, then substitute the window) — see the in-book note |
