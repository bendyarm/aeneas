...
| `aes_fixslice_keyexpand.lisp` | handwritten | **Phase 4 — key-schedule CORE == Kestrel key expansion, bitsliced, for all inputs**: the main theorem `core-windows-are-bitslice-of-keyexpansion` — running the extracted seed (`bitslice(key,key)` into window 0) and the ten `key_round`s fills **every** window r (r = 0..10) with `bitslice(kk_r,kk_r)`, `kk_r = kk-iter(key,r)` the iterated AES recurrence. Built from `step-star` (c-uniform both-lanes step), `inp-of-kk-iter`, the core model `kr-chain` + invariant `wok`, `wok-of-key-round-below` (a round preserves windows at/below its offset — proved via the congruence `wok-cong` (wok reads only `rd8`) so the huge `key_round` term never enters the `wok` induction), `wok-of-key-round` (extends the invariant one window via `key-round-window` + `step-star`), and `kr-chain-wok` (threads all 10 rounds, `off = 8i` via `key-round-car`). Runs on ground-zero linear arithmetic (arithmetic-5 explodes the 8-strided offset inductions; its frame primitives are isolated in keyframe). Remaining for the full schedule: compose the fold layer (`sub_bytes_nots`=xor63, `inv_shift_rows_i`=isr^i, already proven per-op) onto these core windows |
| `aes_fixslice_keyfold.lisp` | handwritten | **Phase 4 — key-schedule fold layer, all inputs**: the fold math for every `(inp b)`. `star` facts — each fold op preserves the symmetric-bitslice form: `inv_shift_rows_i(bitslice(b,b)) = bitslice(isr^i(b),isr^i(b))`, `sub_bytes_nots(bitslice(b,b)) = bitslice(xor63(b),xor63(b))` (`xor63 = ^0x63`; `inv_shift_rows^2` byte-transform defined here). `fk-read` facts — a folded window reads lane 0 back to its `fk` value: `isr_i` then `sub_bytes_nots` → `xor63(isr^i(b))`, `sub_bytes_nots` only → `xor63(b)`, no op → `b`. GL per op/window, lifted to `(inp b)`. With `core-windows-are-bitslice-of-keyexpansion` this is the complete fold correspondence `fk(r) = xor63(isr^{r mod 4}(kk_r))`; remaining is the read-over-write threading of the 17 concrete at-ops over the 11 windows |
