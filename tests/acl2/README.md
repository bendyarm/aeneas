...
| `aes_fixslice_keycrux.lisp` | handwritten | **Phase 4 — connecting `krw8` to the spec recurrence (round 0)**: `krw8-crux-0` proves `car(inv_bitslice(krw8(bitslice(b,b),0))) = kr-spec-bytes(b,1)` for every 16-byte `b`, by combining `key-round-step-rcon0` (keycore) with `key-round-window` (keyround) over the seed `rk0 = append(bitslice(b,b), 80 zeros)`. Reusable bridges: `take-8-nthcdr = rd8`, `bitslice`-of-`inp` is a `wstate`, `rd8` of the seed reads back the `bitslice`. Rounds 1–9 are structurally identical but the same `:use` replay explodes in the rewriter (~184 s prove, then fails) for `c≥1` while `c=0` closes in <1 s — a robust c-general replacement is the next step (see in-book note) |
