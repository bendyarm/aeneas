...
let extract_file (config : gen_config) (ctx : gen_ctx) (fi : extract_file_info)
  ...
  if Config.backend () = Acl2 then begin
    (* The ACL2 backend has a dedicated sexp printer which does not go
       through the Format-based extraction machinery *)
    ExtractAcl2.extract_crate out fi.rust_module_name ctx;
   ...
   *)
    ...
    | Lean ->
        ...
        (* Add the custom imports *)
        List.iter
          (fun m -> Printf.fprintf out "import %s\n" m)
          fi.custom_imports;
        (* Add the custom includes *)
        List.iter
          (fun m -> Printf.fprintf out "import %s\n" m)
    ...
    | Acl2 -> () (* unreachable: dedicated printer above *)
  ...
  let namespace, crate_name, full_dest_dir, extract_filebasename =
    ...
    | Some filename ->
        ...
        let crate_name =
          ...
          else if Config.backend () = Acl2 then
            (* Keep the snake-case basename: ACL2 book names are filenames *)
            basename
          else crate_name
        in
        ...
        let namespace =
          ...
          | None -> (
              match Config.backend () with
              | FStar | Coq | HOL4 | Acl2 -> crate.name
              | Lean -> crate.name)
        in
        let full_dest_dir =
          match subdir with
